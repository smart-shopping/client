...
function sendData () {
  ...
  $.ajax({
    url       : "http://localhost:3000/api/music",
    type      : "post",
    data      : {sourceImage: $("#url").val()},
    success   : (respond) => {
      console.log(respond);
      console.log('============================');
      const playList = JSON.parse(respond).playlists.items[0].external_urls.spotify
      console.log(playList);
      // $("#frame-player").attr("src",playList)
      $("#player").append(`<a href=${playList} class='button'>link to playlist</a>`)
    },
    error     : (err) => {
      console.log(err);
    }
